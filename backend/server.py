    ...
    for schedule in meal_schedule:
        ...
        if preferred_foods["glucides"]:
            ...
            if remaining_carb > 0:
                ...
                # Find equivalents for carbs
                equivalents = find_equivalents(carb_food, user_prefs, schedule["nom"])
                equiv_list = []
                for eq in equivalents:
                    eq_quantity = (item_carb / eq["glucides_100g"]) * 100 if eq["glucides_100g"] > 0 else 0
                    eq_quantity = round(eq_quantity / 10) * 10
                    
                    if eq["unite_personnalisee"]:
                        units_needed = max(1, round(item_carb / eq["glucides_100g"]))
                        qty_display = f"{units_needed} {eq['unite_personnalisee'].replace('1 ', '')}" if units_needed > 1 else eq['unite_personnalisee']
                    else:
                        qty_display = f"{int(eq_quantity)}g"
        ...
        if preferred_foods["lipides"] and not skip_separate_lipids:
            ...
            if remaining_fat > 0:
                ...
                # Find equivalents for fats
                equivalents = find_equivalents(fat_food, user_prefs, schedule["nom"])
                equiv_list = []
                for eq in equivalents:
                    eq_quantity = (item_fat / eq["lipides_100g"]) * 100 if eq["lipides_100g"] > 0 else 0
                    eq_quantity = round(eq_quantity / 5) * 5
                    
                    if eq["unite_personnalisee"]:
                        units_needed = max(1, round(item_fat / eq["lipides_100g"]))
                        qty_display = f"{units_needed} {eq['unite_personnalisee'].replace('1 ', '')}" if units_needed > 1 else eq['unite_personnalisee']
                    else:
                        qty_display = f"{int(eq_quantity)}g"
